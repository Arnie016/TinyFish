...
export function TinyFishLiveView({ events, progress, isActive }: TinyFishLiveViewProps) {
  const currentUrl = events.find((e) => e.type === 'PROGRESS')?.message.includes('Navigating')
    ? events.filter((e) => e.message.includes('Navigating')).pop()?.message.replace('Navigating to ', '') ?? ''
    : 'about:blank';
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        {events.find((e) => e.type === 'STREAMING_URL') && (
          <span className="text-[10px] font-mono text-cyan px-2 py-0.5 rounded bg-cyan/10 border border-cyan/20">
            Session Live
          </span>
        )}
      </div>
